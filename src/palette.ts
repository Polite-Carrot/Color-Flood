...
export type Colour = {
  ...
  mark: string;
  name: string;
  /* For a shared result, where the only thing that renders everywhere is an
     emoji. Unicode has nine coloured squares and this palette has ten
     colours, so the last four share what is left — which costs nothing in
     practice, because a daily never reaches past the sixth. */
  square: string;
};
