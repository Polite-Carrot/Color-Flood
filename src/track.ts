...
export function trim(params: Params): Params {
  const out: Params = {};
...
type Consent = { type: string; status: 'GRANTED' | 'DENIED' };
type Firebase = {
  setEnabled(o: { enabled: boolean }): Promise<void>;
  setConsent?(o: { consents: Consent[] }): Promise<void>;
  logEvent(o: { name: string; params?: Params }): Promise<void>;
};

const win = (): (Window & { Capacitor?: { registerPlugin?: (n: string) => Firebase;
  Plugins?: { FirebaseAnalytics?: Firebase } }; gtag?: (...a: unknown[]) => void;
  dataLayer?: unknown[] } & Record<string, unknown>) | null =>
...
export const Track = {
  ...
  injected: false,
  on: false,
  plugin: null as Firebase | null,

  native(): boolean { return !!win()?.Capacitor; },
  ...
  firebase(): Firebase | null {
    ...
    if (!c) return null;
    if (c.registerPlugin) this.plugin = c.registerPlugin('FirebaseAnalytics');
    else if (c.Plugins?.FirebaseAnalytics) this.plugin = c.Plugins.FirebaseAnalytics;
    return this.plugin;
  },
  ...
  async start(ads: boolean): Promise<void> {
    ...
    const gtag = (...a: unknown[]): void => { w.dataLayer!.push(a); };
    w.gtag = gtag;
  ...
  async stop(): Promise<void> {
    ...
    const w = win();
    if (w && MEASUREMENT_ID) w['ga-disable-' + MEASUREMENT_ID] = true;
  },
  ...
  event(name: string, params: Params = {}): void {
    ...
    const w = win();
    if (!w?.gtag) return;
    try { w.gtag('event', name, params); } catch { /* never break play */ }
  },
};
