...
export function styleFor(fullness: number): 'LIGHT' | 'MEDIUM' {
...
export const Haptics = {
  ...
  get(): Plugin | null {
    if (this.broken || !this.on) return null;
    ...
    const cap = (window as unknown as { Capacitor?: { isNativePlatform?: () => boolean;
      registerPlugin?: (n: string) => Plugin } }).Capacitor;
  ...
  fire(run: (p: Plugin) => Promise<void>): void {
    const p = this.get();
    if (!p) return;
    try {
