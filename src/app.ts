...
type Saved = {
  ...
  /* The two consent answers. NULL means not asked yet, which is a third state
     and has to be: "off because they said no" and "off because nobody has
     asked" want completely different things to happen next. A boolean with a
     default would have quietly made the second look like the first. */
  ads: boolean | null;
  stats: boolean | null;
};
...
function finish(): void {
  ...
  const share = $('win-share') as HTMLButtonElement;
...
function playCampaign(mode: Mode, n: number, button: HTMLButtonElement): void {
  ...
  if (saved.mode !== mode) { saved.mode = mode; save(); }
  dealing(button, () => {
    let entry;
    try {
...
function wire(): void {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape') return void goBack();
...
async function offerShare(button: HTMLButtonElement, text: string): Promise<void> {
...
/* Says so on the button itself for a moment. A status line at the bottom of
   ...
   modal. */
function said(button: HTMLButtonElement, what: string): void {
  const was = button.textContent;
...
/* The first level of a campaign nobody has finished yet, or 0 when the whole
   thousand is done. */
function nextUnplayed(mode: Mode): number {
  const best = saved.progress[mode].best;
  for (let n = 1; n <= CAMPAIGN_LENGTH; n++) if (!(best[n - 1] > 0)) return n;
  return 0;
}
...
function resumeAt(): { mode: Mode; n: number } | null {
  const started = (m: Mode) => saved.progress[m].best.some((b) => b > 0);
  const order: Mode[] = saved.mode === 'merge' ? ['merge', 'flood'] : ['flood', 'merge'];
  for (const mode of order) {
    if (!started(mode)) continue;
...
function paintHome(): void {
  ...
  const at = resumeAt();
  const button = $('go-continue') as HTMLButtonElement;
  button.hidden = !at;
  const tag = document.querySelector('.masthead__tag');
  if (tag instanceof HTMLElement) tag.hidden = !!at;
  if (!at) return;
...
function goBack(): boolean {
  ...
  for (const id of ['overlay-win', 'overlay-howto', 'overlay-settings']) {
    if (!$(id).hidden) { closeOverlay(id); return true; }
  }
  if (!$('overlay-privacy').hidden) return true;   /* swallowed, not closed */
  if ($('screen-game').classList.contains('is-active')) { leaveBoard(); return true; }
  if ($('screen-home').classList.contains('is-active')) return false;
  show('screen-home');
  return true;
}
...
function askConsent(): boolean {
  if (saved.stats !== null) return true;
  /* Nothing to consent to: the web build with no measurement id has no
     ...
     front of the game. */
  if (!Track.configured() && !Ads.native()) return true;
  paintPrivacy();
  openOverlay('overlay-privacy');
  return false;
}
...
(() => {
  ...
  const cap = (window as unknown as { Capacitor?: { isNativePlatform?: () => boolean;
    registerPlugin?: (n: string) => AppPlugin } }).Capacitor;
