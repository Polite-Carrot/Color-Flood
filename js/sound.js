...
export const Sound = {
    ...
    blip(freq, dur, type = 'sine', gain = 0.09) {
        if (!this.on)
            return;
    ...
    flood(fullness) { this.blip(320 + fullness * 260, 0.11, 'sine'); },
    /* Any button. */
    tap() { this.blip(540, 0.06, 'triangle', 0.05); },
    /* A move that could not be played, or one that was wasted. */
    nope() { this.blip(150, 0.13, 'sawtooth', 0.05); },
    win() {
