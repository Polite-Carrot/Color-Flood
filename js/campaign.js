...
function parse(rows) {
    const grid = rows.map((row) => Array.from(row.replace(/\s/g, '')).map((ch) => {
        ...
        if (index < 0)
            throw new Error('no colour has the mark "' + ch + '"');
        return index;
    }));
    const width = grid[0].length;
    for (const row of grid) {
        if (row.length !== width)
            throw new Error('the rows of a drawn board must be the same length');
    }
    return grid;
}
...
function fromDrawn(mode, n, spec) {
    const grid = parse(spec.rows);
    const height = grid.length;
    const width = grid[0].length;
    const origins = mode === 'merge'
    ...
    for (let c = 0; c <= most; c++) {
        if (!used.has(c)) {
            throw new Error('the drawn board for ' + mode + ' level ' + n + ' uses colour ' + most +
